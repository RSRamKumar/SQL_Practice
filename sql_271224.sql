-- Findout the average ratings for each genre
...
with cte as (
select m.genre, m.title,  round(avg(r.rating),2)  avg_ratings,
dense_rank() over(partition by m.genre order by round(avg(r.rating),2) desc) as rank
from movies m 
join reviews r 
on m.id = r.movie_id
...
select genre, title, avg_ratings--,  RPAD('', round(avg_ratings), '*')
